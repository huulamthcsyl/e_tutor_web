...
import { ExamItem, fetchExam, deleteExam } from "@/services/examService";
...
export default function ExamDetailPage() {
  ...
  useEffect(() => {
    const loadExam = async () => {
      try {
        ...
        setError(error instanceof Error ? error.message : "Không thể tải thông tin bài kiểm tra.");
      } finally {
        setLoading(false);
      }
    };
  ...
  if (error || !exam) {
    return (
      <div className="min-h-screen flex items-center justify-center">
        <div className="text-center">
          <p className="text-red-600">{error || "Không tìm thấy bài kiểm tra."}</p>
          <Link href="/admin/exams" className="text-blue-600 hover:text-blue-800 mt-4 inline-block">
            Quay lại danh sách
          </Link>
        </div>
      </div>
    ...
    <div className="container mx-auto px-4 py-8">
      ...
      <div className="bg-white shadow rounded-lg p-6 mb-6">
        <div className="grid grid-cols-2 gap-6">
          <div>
            <h2 className="text-lg font-semibold text-gray-900 mb-4">Thông tin bài kiểm tra</h2>
            <div className="space-y-4">
              ...
              <div>
                ...
                <div className="mt-1 text-gray-900">{formatTimestamp(exam.startTime)}</div>
              </div>
              <div>
                <span className="text-sm font-medium text-gray-500">Thời gian kết thúc:</span>
                <div className="mt-1 text-gray-900">{formatTimestamp(exam.endTime)}</div>
              </div>
              <div>
              ...
              {exam.createdAt && (
                <div>
                  ...
                  <div className="mt-1 text-gray-900">{formatTimestamp(exam.createdAt)}</div>
                </div>
              )}
            </div>
          </div>
